...
def get_actor():
    ...
    outputs_0 = lower_bound[0] + (outputs[:, 0] + 1.0) / 2.0 * (upper_bound[0] - lower_bound[0])
    outputs_1 = lower_bound[1] + (outputs[:, 1] + 1.0) / 2.0 * (upper_bound[1] - lower_bound[1])
...
def load_model_weights(out_dir):
    ...
    if dir_list:
        ...
        return out_dir+filename, max_str
    return None, max_str
...
# # load weights
aw, max_str = load_model_weights(actor_folder)
print(f"aw: a{aw}")
if aw !=None:
    actor_model.load_weights(aw)

cw,_ = load_model_weights(critic_folder)
if cw !=None:
    critic_model.load_weights(cw)
###################################################################
...
buffer = Buffer(50_000, 128)

feedback = Feedback(folder_name)
...
for ep in range(total_episodes):
    ...
    while True:
        ...
        print(f'step: {step}/{1000} episode: {ep}/{total_episodes} goal_reached:{cumulative_goal_reached_count}')
        ...
        if done:
            ...
            rospy.logwarn("********************************************")
            print("episode: {}/{}, score: {}, total reward: {}, goal reached: {} angle_exceed:{} dist_exceed:{}".format(ep, total_episodes, step, episodic_reward,cumulative_goal_reached_count, is_angle_exceed_count, is_dist_exceed_count))
            rospy.logwarn("********************************************")
    ...
    critic_model.save_weights(critic_folder+weight_filename)
