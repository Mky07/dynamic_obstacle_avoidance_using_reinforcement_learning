...
class TD3Agent:
    def __init__(self, env_name, parent_dir, buffer_capacity=50_000, batch_size=64, critic_lr=0.001, actor_lr = 0.0001):
        ...
        self.config['critic_loss'] = self.config['critic_loss'][:size]
        self.config['actor_loss'] = self.config['actor_loss'][:size]

        self.print_done_counts()
        self.draw(tt(self.config['done_reasons'], 'GOAL_REACHED'), 500, 'Episode', 'Goal Reached')
        self.draw(tt(self.config['done_reasons'], 'COLLISION_DETECTED'), 500, 'Episode', 'Collision Detected')
        self.draw(tt(self.config['done_reasons'], 'DIST_EXCEEDED'), 500, 'Episode', 'Distance Exceeded')
        self.draw(tt(self.config['done_reasons'], 'ANGLE_EXEEDED'), 500, 'Episode', 'Angle Exceeded')

        self.draw(self.config["cumulative_rewards"], 500)
        self.draw(self.config["critic_loss"],5000,"Step" ,"Critic Loss")
        self.draw(self.config["actor_loss"],5000,"Step" ,"Actor Loss")
