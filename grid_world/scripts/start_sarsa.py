...
if __name__ == '__main__':
    ...
    filename = "/home/mky/rl_ws/src/openai_examples_projects/dynamic_obstacle_avoidance_using_reinforcement_learning/grid_world/models/model_sarsa1.pkl"
    ...
    plt.show()
    plt.plot(moving_average(log_values['sum_q'], 10))
    plt.show()
    ...
    for x in range(nepisodes):
        ...
        print("sum q: {}".format(log_values['sum_q'][-100:]))
        ...
        state1 = ''.join(str( observation1))
        action1 = qlearn.chooseAction(state1)
        ...
        for i in range(nsteps):
            ...
            observation2, reward, done, info = env.step(action1)
            state2 = ''.join(str( observation2))

            # Choosing the next action
            action2 = qlearn.chooseAction(state2)

            # Learn the Q-value
            qlearn.learn(state1,action1, reward, state2, action2)
        ...
        with open(filename, 'wb') as f:
            ...
            log_values['sum_q'].append(sum(qlearn.q.values()))
            pickle.dump(log_values, f)
